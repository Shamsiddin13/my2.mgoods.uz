...
require_once  __DIR__.'/RetailCRM.php'; //1. Подкючить данный файл
$site_code = 'm-plus'; //2. Выбрать необходимый магазин https://mgoods.retailcrm.ru/admin/sites и указать его символьный код
$item_id = 'mpg651'; // 3. Указать "Внешний код" торгового предложения из карточки нужного товара


$integration = new RetailCRM($site_code,$item_id);
$order = $integration->getOrderFromPost();
if($integration->isDuplicate($order)){
    header("Location:/rcrm/fqg623/dublru.html"); //5. Указать нужный адрес для перенаправления в случае дубля
    //require __DIR__.'/dubl.html'; //или загрузить файл
    die();
}
...
fbq('init', '745822382588505');
fbq('track', 'PageView');
</script>
<noscript><img height="1" width="1" style="display:none"
src="https://www.facebook.com/tr?id=745822382588505&ev=PageView&noscript=1"
/></noscript>
<!-- End Meta Pixel Code -->
...
<a style="color: white;" href="https://mgoods.uz">Buy</a>
	...
	input:hover{
	  border-color: rgba(47, 99, 144, 0.6);
	}
	button.button7 {
	  display: block;
	  font-weight: 900;
	...
	button.button7:hover { background: rgba(84, 99, 144, 0.5); }
	...
	.error {color: #FF0000;}
	.alert {margin: 15px 0;padding: 15px 20px;text-align: center;}
	.alert-danger {border: 1px solid #FF0000;background-color: #FFFCFC; color: #FF0000;}
	.alert-danger a{color: #FF0000;text-decoration: none;}
...
/*.row .card .img{*/
/*    border-bottom:solid thin  #35a76e;*/
/*}*/
.row .card a{
    color:#35a76e;
    text-decoration: none;
