...
class ProductResource extends Resource
{
    ...
    public static function getNavigationBadge(): ?string
    {
        return static::getModel()::query()
            ->join('landing', 'products.article', '=', 'landing.article') // Join the landing table based on article
            ->where('products.status', 'Ochiq') // Filter by the status of the product
            ->count(); // Count all rows that match the conditions
    }
                    ...
                    Tables\Columns\Layout\Stack::make([
                        ...
                        Tables\Columns\TextColumn::make('two_plus_one')
                            ...
                            ->formatStateUsing(function (?string $state) {
                                // Check if the state is exactly 'Bor'
                                if ($state === 'Bor') {
                                    return '<span class="font-bold">2+1: </span>' . e($state);
                                }

                                // Return an empty string or any alternative display if not 'Bor'
                                return '';
                            })
                            ->html(),
                    ]),
            ...
            ->actions([
                ...
                Tables\Actions\Action::make('create_stream')
                    ...
                    ->action(function ($record, $livewire) {
                        ...
                        $livewire->dispatchBrowserEvent('submitStreamForm', [
                            'url' => route('streams.store'), // The route for form submission
                            'landing_id' => $record->landing_id, // Pass the landing_id
                        ]);
                    })
                    ->modalHeading('Oqim olish') // The modal heading
                    ->tooltip("Oqim olish")
                    ->button() // The submit button label
                    ->modalSubmitActionLabel("Yaratish")
...
//                        ]);
//                        $stream->save();
//                        dispatch(function () use ($data, $stream) {
//                            Notification::make()
...
//                                ->send()
//                                ->actions([
//                                    Action::make('goToStream')
//                                        ->label("Oqimlar")
//                                        ->tooltip("Barcha oqimlar ro'yhatiga o'tish")
//                                        ->url(fn() => ListStreams::getUrl()),
//                                ]);
...
//                            Notification::make()
//                                ->title("Yangi Oqim uchun Link Muvaffaqiyatli olindi")
//                                ->body("Oqim nomi: **{$data['stream_name']}**")
//                                ->success()
//                                ->send();
//                        })->onQueue('notifications');
//                    }),
                ...
                Tables\Actions\Action::make('oqim_link')
                    ->tooltip('Ohirgi yaratilgan oqim linki')
