...
class OrderResource extends Resource
{
    ...
    {
        return parent::getEloquentQuery()->where('store', auth()->user()->store);
    }
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                ...
                TextColumn::make('displayProductName')->label('Mahsulot nomi')->searchable()->toggleable()->sortable(),
                ...
                TextColumn::make('statusUpdatedAt')->label("Oxirgi o'zgarish")->sortable()->toggleable()->dateTime(),
                TextColumn::make("buyPrice")
                    ->label("Kassa narxi")
                    ->sortable()
                    ->getStateUsing(fn ($record) => Product::where('article', $record->article)->value('buyPrice'))
                    ->formatStateUsing(fn($state) => number_format($state, 0, '.', ' ')),
                TextColumn::make('store')->label('Store')->searchable()->toggleable()->sortable(),
            ])
            ->defaultSort("statusUpdatedAt", 'desc')
            ->paginated([
                10,
                15,
                25,
            ...
            ->filters([
                Tables\Filters\SelectFilter::make('article')
                    ->options(Order::getAllArticleGroupedBy('store', auth()->user()->store)->pluck('article', 'article')->toArray())
                    ->label('Artikul')
                ...
                Tables\Filters\SelectFilter::make('displayProductName')
                    ->options(Order::getAllProductNameGroupedBy('store', auth()->user()->store)->pluck('displayProductName', 'displayProductName')->toArray())
                    ->label('Mahsulot Nomi')
